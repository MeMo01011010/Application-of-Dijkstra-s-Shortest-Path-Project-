...
Hours spent making and testing your min heap class: 3
...
void minHeap<T>::insert(int* curDist, int* locator,const T& el){
   ...
   // this->num++; // increment number of elements
    //int p = (this->num-1) / 2; // set p equal to parent of new element
  ...
  if(smallest != i)
    {
      ...
      mySwap(this->ar[smallest], this->ar[i]);
     // mySwap(locator[ar[smallest]], locator[ar[i]]);
...
void minHeap<T>::bubbleUp(int* curDist, int* locator, int i){
    ...
    while(curDist[ar[i]] < curDist[ar[p]] && i > 0){//while the new element is larger than it's parent
      ...
      mySwap(this->ar[i], this->ar[p]); //swap elements
...
void minHeap<T>::remove(int* curDist, int* locator, int i){
    ...
    mySwap(this->ar[i], this->ar[this->num-1]); // swap last element with element to be removed
